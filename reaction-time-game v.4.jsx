...
function ReactionTest() {
  ...
  const randomTimeDelayRef = useRef(0);
  ...
  const onRedBoxPress = () => {
    setGameState("failed");
    clearTimeout(randomTimeDelayRef.current);
    clearInterval(timeDifferenceRef.current);
  };
  ...
  const onGreenPress = () => {
    ...
    clearTimeout(randomTimeDelayRef.current);
    clearTimeout(timeDifferenceRef.current);
  };

  // for managing the time that is running
  useEffect(() => {
    if (gameState === "reactionReady") {
      timeDifferenceRef.current = setTimeout(() => {
        setReactionTime(Math.floor(performance.now() - reactionStartTime));
      }, 1);
    }
  }, [reactionTime, gameState]);
  ...
  useEffect(() => {
    if (gameState === "playing") {
      ...
      randomTimeDelayRef.current = setTimeout(() => {
  ...
  return (
    <div className="flex flex-col justify-center items-center p-4">
      ...
      {gameState === "reactionReady" && <span className="px-4 py-4 font-bold">{`${reactionTime} ms`}</span>}
